...
FIT = {                      # 역할 → (A_max, C_k, C_k 를 적합한 상류압, k_shape)
    ...
    # 음압 채널은 2026-08-28 실기 20260828_173347 로 **처음 실측**했다.
    # 그 전에는 양압 값에 1.778125/0.58789258 = 3.02 를 곱한 추정치였고, 두 밸브가
    # 서로 반대 방향으로 틀려 있었다:
    #   n1 micro (챔버→진공, 압력 내림): 실측 크래킹 115 mA / A_max 0.0185.
    #     모델은 크래킹 107 mA / A_max 0.0976 (5.3 배) 로 봐서 너무 조금 열었고,
    #     깊은 목표(49.8·52.0 kPa)에서 오차가 +15 kPa 로 남았다.
    #   n2 atm (대기→챔버, 압력 올림): 실측 105 mA 에서 이미 면적 0.0105.
    #     모델은 반개를 142 mA 로 봐서 너무 많이 열었고 +20 kPa 오버슛했다.
    # 이 조합이 목표가 깊을수록 커지는 ±20 kPa 진동의 정체다.
    #
    #   n1 실측: 105mA→0.000074  115→0.001994  125→0.011906  135→0.015008  145→0.018026
    #   n2 실측: 105mA→0.010488  115→0.013286  125→0.014177  135→0.014704
    ('neg', 'micro'): (0.018500, 0.13251600,  80.0, K_SHAPE),
    ('neg', 'atm'):   (0.015000, 0.11419700, 101.3, K_SHAPE),
    ('neg', 'macro'): (0.018500, 0.13251600,  80.0, K_SHAPE),
}
...
PIN_ROLE = {
    ...
    ('neg', 'micro'):  80.0,   # 챔버 → 음압 레일 (상류=챔버, 진공 동작 중 대표값)
    ('neg', 'atm'):   101.3,   # 대기 → 챔버
    ('neg', 'macro'):  80.0,   # 챔버 → 이젝터 (상류=챔버)
}

...
★ 20260903 이후 이 도구는 **CAN 엔코더(보드 17~22) 전용이고 지금은 안 쓰인다.**
  엔코더는 Teensy 로 옮겼다. Teensy 채널의 2점 보정은
      python3 scripts/pp_check.py --calib
  로 잡는다 (0° 에서 [0], 90° 에서 [9], [p] 로 config 줄 출력).
  이 파일은 encoder_source:can 으로 되돌릴 때를 위해 남겨 둔다.

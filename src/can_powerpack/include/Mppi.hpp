...
struct Params {
  ...
  // **동시 개방(공급 micro + 배기 atm) 벌점.**
  //
  // 압력을 유지하는 (micro, atm) 조합은 **하나가 아니다** — 순유량만 같으면
  // 어떤 쌍이든 추종 비용이 똑같다. 그 축퇴 방향에 벌점이 없어서, 채널마다
  // 초기값·누설에 따라 "둘 다 활짝 열고 밀당하는" 해로 흘러갔다.
  // 그 상태는 순유량이 큰 값 두 개의 차라 dP/du 감도가 매우 커서 한계진동한다.
  //
  // 실측 20260904_122116 (유지 구간 36 건, min(micro,atm) 로 정렬):
  //   동시개방 ≥ 15 % 인 11 건이 **전부** 진동 (압력 σ 4.8~9.1 kPa, 1.9~2.6 Hz)
  //   진동 13 건 평균 15.4 %  vs  정숙 23 건 평균 3.6 %
  //
  // min(micro, atm)² 를 벌한다 — 한쪽만 열면 0 이라 과도 응답은 건드리지 않는다
  // (과도 구간에 두 밸브를 동시에 여는 것은 어차피 이득이 없다).
  // 0 으로 두면 이 항이 꺼진다.
  float w_coact{0.0f};

...
private:
  ...
  bool integ_hold_on_band_{false};
  // **폭주 보호** [deg]. 팔이 목표보다 이만큼 위에 있으면서 아직 올라가고
  // 있으면 요구 토크를 그 자세의 중력 아래로 묶어 강제로 감속시킨다.
  // 실기 20260904_163842 에서 목표 20° 인데 102° 까지 올라갔다 — 그때까지
  // 개입하는 것이 아무것도 없었다. 0 이하면 끔.
  double angle_runaway_deg_{5.0};
  // 폭주 시 실제 자세 중력토크에 곱할 비율. 1 미만이어야 하강 감속 여유가 생긴다.
  double runaway_gravity_ratio_{0.5};
  ...
  // 스트로크 → 부피 배수. **부피식에만** 곱한다 (토크 환산 F = τ/reel 에는 안 쓴다).
  //
  // 이 액추에이터는 단순 피스톤이 아니다 — 수축하면서 **옆면도 같이 좁아진다.**
  // 그래서 부피 변화가 A_바닥 × 스트로크 보다 크다. 기준 부피(θ=0)는 그대로 두고
  // 변화율만 키우려면 스트로크에 배수를 곱하면 된다:
  //     V = tank + A × max(0, offset ± mult·x)
  // 1.0 이면 예전 동작(단순 피스톤).
  //
  // 부피는 dP/dt = ṁ·R·T/V 의 분모라 **그대로 내층 이득**이다. 과대평가하면
  // 모델이 유량을 더 요구해 과개방하고, 과소평가하면 굼뜬다. 하나씩 바꿀 것.
  double stroke_vol_mult_{1.0};

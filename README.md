...
브랜치 `feat/mppi-controller`. **실기 피팅은 밸브·펌프 모두 완료해 `config/` 에 들어갔다.
그런데 실기 제어는 아직 안 된다 — 압력이 목표(178 kPa)를 넘어 550 kPa 까지 뱅뱅한다.**

> **실기에서 이어서 작업한다면 [`HANDOFF.md`](HANDOFF.md) 를 먼저 읽을 것.**
> 무엇을 고쳤고 무엇이 왜 아직 안 되는지, 다음 실험이 무엇인지가 거기 있다.
> 이 절은 요약이다.

| 영역 | 상태 |
|---|---|
| 제어 파이프라인 (mode 2) | 시뮬 동작. 채널 MPC 는 **MPPI**(선형화 없음, [MPPI.md](MPPI.md)). 6축 다단 스텝 정착 1.13 s, 압력 RMSE 5.9 kPa — QP 대비 IAE −62% |
| **실기 압력 추종** | **안 됨 — 뱅뱅/진동.** macro 부스트 게이트 2개가 유력 용의자 (HANDOFF 3-1, 3-2) |
| 밸브 13-parameter (12채널 × 3) | **실측 피팅 완료** → `config/valve_params.yaml`. 단 Debug(PWM) 모드에서 피팅했고 지금은 Normal(PID) 모드다 (HANDOFF 3-5) |
| 레일 부피 · 누설 | **실측 완료.** pos 138.6 / neg 29.0 mL, 0.00108 / 0.00005 LPM/kPa |
| 펌프 실측 능력경계 | **실측 완료 + C++ 반영 경로 신설.** 기존 하드코딩 1200 kPa 는 실측(335~386)의 3~4배 과대평가였다 |
| 펌프 `PumpGeom` (기하) | 실측 피팅했으나 **신뢰도 낮음** (소기량×Cb_in 축퇴). 능력경계는 위 측정 테이블을 쓴다 |
| 오리피스 `Cd` · `valve_open_eta` | **추정값.** `Cd=0.8` 표준 가정, `eta=1.0`(물리값 0.75, 켜면 재튜닝 필요) |
| 센서 캘리브레이션 | 압력 보드 1–16 있음(board7 재영점 완료). **엔코더 board 20·21·22 없음** — 기본값 gain 이 실측 3개와 **부호 반대** → 그 축은 각도가 거꾸로 읽힌다 |
| 밸브별 파라미터 로더 (C++) | 완료. 기동 로그가 `N/N 채널 로드` 를 찍는다 |
| gid11 (board16) | **하드웨어 무응답** — 소프트웨어로 못 고침, 물리 점검 필요 |

**다음에 할 일 (순서)**

1. **macro 게이트 두 개를 동시에 끄고 micro 단독 추종 확인** — HANDOFF 3-2 의 미완 실험.
   조용해지면 원인은 게이트 정책, 여전히 폭주하면 피드포워드 역모델이나 13-parameter 자체다.
2. 엔코더 board 20·21·22 2점 캘리브레이션 (6축 위치 제어의 전제, **부호가 반대라 안전 문제**).
   `scripts/encoder_calib.py`, RUNBOOK 0.5절.
3. `control_mode=0` 의 QP 100% 실패 원인 규명 (HANDOFF 3-3).
4. 1축이 안정되면 `num_actuators` 1→2→3→6, 각 단계에서 `solver` qp/mppi/mppi_system 비교.

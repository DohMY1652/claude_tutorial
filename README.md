...
| 영역 | 상태 |
|---|---|
| 제어 파이프라인 (mode 2) | 동작. 채널 MPC 는 **MPPI**(선형화 없음). 6축 45° 스텝 정착 2.4 s, 최종오차 0.09° |
...
**성능 — 6축 45° 스텝, 하네스 비결정론 때문에 반복 측정**

| | 오버슈트 [°] | 정착 [s] | 최종오차 [°] |
|---|---|---|---|
| qp (n=6) | 2.36 (1.51~3.12) | 2.72 (2.50~3.14) | 0.151 (0.045~0.210) |
| mppi (n=8) | 1.94 (1.14~2.82) | 2.38 (2.04~3.22) | **0.086 (0.061~0.122)** |

정직하게 읽으면:

- **최종오차는 개선이다.** mppi 의 최댓값(0.122)이 qp 의 평균(0.151)보다 작다.
- **정착도 개선이다.** mppi 8회 중 **6회가 qp 의 최속(2.50 s)보다 빠르다** (qp 는 6회 모두 2.50 이상).
- **오버슈트는 평균이 낮지만(1.94 vs 2.36) 범위가 겹쳐 이 표본으로는 결론을 못 낸다.**
  단일 실행 비교로 판정하면 안 된다 — 0절의 경고가 그대로 적용된다.
...
(이번)   펌프 리허설로 찾은 결함 7건 수정 (안전 3 + 솔버 4)
(이번)   실기 피팅 절차서(RUNBOOK.md)
(이번)   채널 MPC 를 선형화 QP → MPPI 로 교체 (feat/mppi-controller)
```
...
src/can_powerpack/
  ...
  include/Mppi.hpp, src/Mppi.cpp     채널 MPC 의 MPPI 솔버 (2절 3-1, 8.10)
